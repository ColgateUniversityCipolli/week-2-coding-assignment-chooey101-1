...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hldef{right.int} \hlkwb{<-} \hlkwd{integrand}\hldef{(right.points)}
\hldef{left.int} \hlkwb{<-} \hlkwd{integrand}\hldef{(left.points)}
\hldef{(area} \hlkwb{<-} \hlkwd{sum}\hldef{(delta.x}\hlopt{*}\hldef{(right.int}\hlopt{+}\hldef{left.int)}\hlopt{/}\hlnum{2}\hldef{))}
\end{alltt}
\begin{verbatim}
## [1] 8.6664
\end{verbatim}
